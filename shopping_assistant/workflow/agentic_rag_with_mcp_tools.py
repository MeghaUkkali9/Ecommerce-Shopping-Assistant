...
class AgenticRAG:
    ...
    async def __safe_async_init(self):
    ...
    async def __vector_retriever(self, state: AgentState):
        print("--- RETRIEVER (MCP) ---")
    ...
    async def __web_search(self, state: AgentState):
        print("--- WEB SEARCH (MCP) ---")
        
        query = state["messages"][-1].content
        
        tool = next(tool for tool in self.mcp_tools if tool.name == "web_search")
        result = await tool.ainvoke({"query": query}) 
...
if __name__ == "__main__":
    ...
    answer = rag_agent.run("Provide obsessed lipstick")
    print("\nFinal Answer:\n", answer)